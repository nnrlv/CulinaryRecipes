...
public class TaskExecutor : ITaskExecutor
{
    ...
    private async Task Execute<T>(Func<T, Task> action)
    {
        ...
        {
            logger.LogError($"Error: {QueueNames.SEND_EMAIL_RECIPE_SUBSCRIPTION}: {e.Message}");
            throw;
        }
    }
    ...
    public void Start()
    {
        rabbitMq.Subscribe<EmailModel>(QueueNames.SEND_EMAIL_RECIPE_SUBSCRIPTION, async data
            => await Execute<IEmailSender>(async service =>
            {
                logger.LogDebug($"RABBITMQ::: {QueueNames.SEND_EMAIL_RECIPE_SUBSCRIPTION}: {data.Email} {data.Message}");
